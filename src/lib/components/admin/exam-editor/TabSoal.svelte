<script lang="ts">
	...
	import type { Question } from '$lib/services/admin/exam.service';
...
[SOAL 2]
Sebutkan dua warna pada bendera Indonesia!

Warna di bagian atas:
...
[JAWABAN] B`;
	...
	async function importDocx(event: Event) {
		...
		try {
			...
			// Handle mammoth conversion with custom image uploader
			const options = {
				convertImage: mammoth.images.imgElement(function (image) {
					return image.readAsArrayBuffer().then(async function (imageBuffer) {
						try {
							// Create a file from buffer and upload via mediaService
							...
							console.error('Failed to upload image from DOCX:', error);
							return { src: '' };
						}
					});
				})
			...
			if (importedQuestions.length > 0) {
				...
				activeQuestionIndex = questions.length - importedQuestions.length; // Focus the first imported question
			} else {
...
<div
	class="mt-4 flex flex-wrap items-center justify-between gap-4 rounded-xl border bg-card p-4 shadow-sm"
>
	<div class="flex flex-wrap items-center gap-2">
		...
		<input
			...
			onchange={importDocx}
		/>
		<Button variant="secondary" onclick={onaddQuestion} title="Tambah Soal Baru">
	...
	<div class="flex items-center gap-4">
		...
		<Button variant="default" class="font-bold shadow-sm" onclick={onsave} disabled={isSaving}>
...
<div class="mt-4 flex min-h-[700px] overflow-hidden rounded-xl border bg-card shadow-sm">
	<div class="flex w-64 flex-col border-r bg-muted/10">
		<div class="flex items-center justify-center border-b bg-muted/20 p-4">
			<span class="text-xs font-bold tracking-widest text-muted-foreground uppercase"
				>Daftar Soal ({questions.length})</span
			>
		</div>
		...
		{#if questions.length > 0 && activeQuestionIndex >= 0 && activeQuestionIndex < questions.length}
			...
			<div class="mb-4">
				<div class="mb-6 flex items-center justify-between">
					...
					<Button variant="outline" size="sm" onclick={onaddOption} class="shadow-sm">
						<Plus class="mr-2 h-4 w-4" /> Tambah Pilihan
					</Button>
				</div>
				<div class="space-y-6">
					{#each currentQ.options as opt, optIdx}
						<div class="flex items-start gap-4">
							<div class="pt-2">
								<input
									...
									class="mt-3 h-5 w-5 cursor-pointer text-primary accent-primary transition-all"
							...
							>
								<div
									class="flex items-center justify-between border-b px-4 py-2 {currentQ.correct_option ===
									String(optIdx)
										? 'bg-primary/10'
										: 'bg-muted/30'} rounded-t-xl transition-colors"
								>
									<label
								...
								<div class="overflow-hidden rounded-b-xl border-0 bg-background p-0">
									<RichTextEditor
		...
		{:else}
			...
			>
				...
				<p class="max-w-sm text-center text-sm">
					Silakan pilih soal dari navigasi di sebelah kiri, atau tekan tombol + untuk mulai membuat
					soal pertama ujian ini.
				</p>
			</div>
		{/if}
	</div>
