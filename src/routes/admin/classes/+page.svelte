<script lang="ts">
	...
	function openEditDialog(classItem: Class) {
		isValues = classItem;
		formGradeLevel = classItem.grade_level;
...
<div class="flex h-full flex-1 flex-col space-y-8 p-8">
	<PageHeader
		...
		description="Kelola daftar tingkat, jurusan, dan rombongan belajar (rombel) siswa."
	>
					...
					{#each classesList as classItem (classItem.id)}
						<Table.Row>
							...
							<Table.Cell class="font-medium"
								>{classItem.grade_level} {classItem.major_code} {classItem.group_number}</Table.Cell
							>
							<Table.Cell class="text-right">
								<div class="flex justify-end gap-2">
									...
									<Button
										...
										onclick={() => openDeleteAlert(classItem)}
									>
		...
		<AlertDialog.Content>
			<AlertDialog.Header>
				...
				<AlertDialog.Description>
					Tindakan ini tidak dapat dibatalkan. Kelas <strong
						>{classToDelete?.grade_level}
						{classToDelete?.major_code}
						{classToDelete?.group_number}</strong
					> akan dihapus permanen dari sistem. Data tidak dapat dihapus jika masih ada siswa yang terdaftar
					di kelas ini.
				</AlertDialog.Description>
			</AlertDialog.Header>
			<AlertDialog.Footer>
