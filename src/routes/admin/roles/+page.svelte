...
<div class="h-full flex-1 flex-col space-y-8 p-8 flex">
    <PageHeader title="Manajemen Peran & Izin" description="Kelola peran administrator dan akses sistem mereka.">
                                    ...
                                    {#if role.permissions && role.permissions.length > 0}
                                        ...
                                        {:else}
                                            ...
                                            {#if role.permissions.length > 5}
                                                <Badge variant="outline" class="mr-1 mb-1">+{role.permissions.length - 5} lainnya</Badge>
                                            {/if}
                                        {/if}
                                    {:else}
    ...
    <Dialog.Root bind:open={isDialogOpen}>
        <Dialog.Content class="sm:max-w-[700px] sm:max-h-[90vh]">
            <Dialog.Header>
        ...
        <AlertDialog.Content>
            <AlertDialog.Header>
                ...
                <AlertDialog.Description>
                    Tindakan ini tidak dapat dibatalkan. Peran <strong>{roleToDelete?.name}</strong> akan dihapus permanen. Penghapusan akan gagal jika peran ini masih digunakan oleh pengguna aktif.
                </AlertDialog.Description>
            </AlertDialog.Header>
            <AlertDialog.Footer>
