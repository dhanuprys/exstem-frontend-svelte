...
<div class="grid max-h-[60vh] gap-4 overflow-y-auto px-1 py-4">
	<div class="grid gap-2">
		...
		{#if isEdit && name === 'Superadmin'}
			<p class="mt-1 text-xs text-muted-foreground">
				Peran sistem Superadmin tidak dapat diubah namanya.
			</p>
		{/if}
	</div>
					...
					<Accordion.Item value={group}>
						<Accordion.Trigger class="py-3 hover:no-underline">
							<div class="flex items-center space-x-2">
								<div class="mr-2 font-medium">{group.toUpperCase()}</div>
							</div>
						</Accordion.Trigger>
						<Accordion.Content>
							<div class="rounded-md border bg-muted/30 p-4">
								<div class="mb-4 flex items-center space-x-2 border-b pb-4">
									<Checkbox
										id={`group-all-${group}`}
										checked={isGroupFullySelected(perms, permissions)}
										onCheckedChange={(v) => toggleGroup(group, perms, v === true)}
									/>
									<Label for={`group-all-${group}`} class="cursor-pointer font-semibold">
									...
									{#each perms as perm}
										<div class="flex items-start space-x-2">
											...
											<div class="grid gap-1.5 leading-none">
												<Label
													for={`perm-${perm}`}
													class="cursor-pointer text-sm leading-snug font-normal"
												>
													{perm}
												</Label>
											</div>
