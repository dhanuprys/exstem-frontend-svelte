<script lang="ts">
    ...
    async function loadData(page = 1) {
        isLoading = true;
        try {
            const classIdParam = selectedClassFilter && selectedClassFilter !== "all" 
...
<div class="h-full flex-1 flex-col space-y-8 p-8 flex">
    <PageHeader title="Manajemen Siswa" description="Kelola data siswa, pengaturan kelas, dan akses ujian.">
                        ...
                        <Table.Row>
                            ...
                            <Table.Cell class="text-right">
                                <div class="flex justify-end gap-2">
                                    <Button variant="ghost" size="icon" title="Reset Sesi Login" onclick={() => openResetAlert(student)}>
                                        <KeyRound class="h-4 w-4" />
                                    </Button>
                                    <Button variant="ghost" size="icon" onclick={() => openEditDialog(student)}>
                                        <Pencil class="h-4 w-4" />
                                    </Button>
                                    <Button variant="ghost" size="icon" class="text-destructive hover:text-destructive" onclick={() => openDeleteAlert(student)}>
    ...
    <Dialog.Root bind:open={isDialogOpen}>
        <Dialog.Content class="sm:max-w-[425px]">
            <Dialog.Header>
                ...
                <Dialog.Description>
                    Isi informasi pelajar pada form di bawah ini.
                </Dialog.Description>
            </Dialog.Header>
            <StudentForm 
                bind:nis={formNIS} 
        ...
        <AlertDialog.Content>
            <AlertDialog.Header>
                ...
                <AlertDialog.Description>
                    Tindakan ini tidak dapat dibatalkan. Data siswa <strong>{studentToDelete?.name} ({studentToDelete?.nisn})</strong> akan dihapus permanen dari sistem.
                </AlertDialog.Description>
            </AlertDialog.Header>
            <AlertDialog.Footer>
    ...
    <AlertDialog.Root bind:open={isResetAlertOpen}>
        <AlertDialog.Content>
            <AlertDialog.Header>
                ...
                <AlertDialog.Description>
                    Apakah Anda yakin ingin mereset sesi untuk siswa <strong>{studentToReset?.name} ({studentToReset?.nisn})</strong>? Tindakan ini akan memaksa keluar akun mereka dari perangkat aktif saat ini.
                </AlertDialog.Description>
            </AlertDialog.Header>
            <AlertDialog.Footer>
                <AlertDialog.Cancel>Batal</AlertDialog.Cancel>
